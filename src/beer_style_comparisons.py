...
def weighted_mean(data, base_mean, weight):
    """
    Helper function for creating a weighted mean
    from two pandas dataframe columns.
...
if __name__ == "__main__":
    ...
    bsdf_grouped = (
        bsdf.groupby("beer_style")
        .agg(
            {
                "beer_style": "count",
                "review_overall": ["std", "mean"],
                "review_aroma": ["std", "mean"],
                "review_appearance": ["std", "mean"],
                "review_palate": ["std", "mean"],
                "review_taste": ["std", "mean"],
            }
        )
        .round(2)
        .reset_index()
    ...
    bsdf_grouped.sort_values(
        ("beer_style", "count"), ascending=False, inplace=True
    )
    print(bsdf_grouped)

    bsdf_grouped.reset_index(inplace=True)

    bsdf_t20 = bsdf_grouped.loc[
        :,
        [
            ("beer_style", ""),
            ("beer_style", "count"),
    ...
    new = bsdf_t20[
        ["beer_style", "review_overall_mean", "beer_style_review_count"]
    ]
    new.sort_values("review_overall_mean", ascending=False)
    # bsdf_high_1 = bsdf_high.loc[:1, :]
    # bsdf_high

    beer_style_weighted_mean = weighted_mean(
        bsdf_t20, "review_overall_mean", "beer_style_review_count"
    )
    ...
    american_ipa_overall_ratings = american_ipa_overall_ratings[
        ["beer_style", "review_overall"]
    ]
    american_ipa_overall_ratings

    american_ipa_mean = round(
        american_ipa_overall_ratings["review_overall"].mean(), 2
    )
    ...
    average_rated_beer_style = weighted_mean(
        bsdf_t20, "review_overall_mean", "beer_style_review_count"
    )
    print(f"Weighted Average: {average_rated_beer_style}")
    unweighted_average_rated_beer_style = round(
        bsdf_t20["review_overall_mean"].mean(), 2
    )
    print(f"Unweighted Average: {unweighted_average_rated_beer_style}")

    test_stat, p_value = stats.ttest_1samp(
        american_ipa_overall_ratings["review_overall"],
        highest_rated_beer_style_mean_rating,
    )
    print(f"Test Statistic:{test_stat}, P-Value:{p_value/2}")
    if (
        p_value / 2
    ) < 0.05:  # alpha value is 0.05 or 5% - one-tailed so divide p-value in half
    ...
    ax.set_xticklabels(
        bsdf_t20.loc[:9, "beer_style"],
        rotation=45,
        ha="right",
        fontweight="bold",
    )
    legend_elements = [
        Line2D([0], [0], color="orangered", lw=20, label="IPAs"),
        Line2D(
            [0],
            [0],
            color="k",
            lw=20,
            label="Darker/Heavier Styles \n (Stouts, Porters)",
        ),
    ...
    bsdf_mean_sort = bsdf_mean_sort.sort_values(
        "review_overall_mean", ascending=False
    )
    ...
    ax.set_xticks(x_data)
    ax.set_xticklabels(
        bsdf_mean_sort.loc[:9, "beer_style"],
        rotation=45,
        ha="right",
        fontweight="bold",
    )
    ...
    ax.boxplot(
        ...
        boxprops=dict(
            linestyle="-", linewidth=2.5, color="k", facecolor="orangered"
        ),
    )
    ...
    plt.savefig(
        "beer_style_ratings_distribution_Amer_IPA.png",
        dpi=300,
        bbox_inches="tight",
    )
    ...
    bsdf_t20["review_overall_rank"] = bsdf_t20["review_overall_mean"].rank(
        axis=0, method="min", ascending=False
    )
    bsdf_t20["review_overall_rank"] = bsdf_t20["review_overall_rank"].astype(
        int
    )
    print(bsdf_t20)

    bsdf_t20["review_overall_percentile"] = (
        (
            1
            - bsdf_t20["review_overall_mean"]
            .rank(axis=0, method="max", ascending=False, pct=True)
            .round(2)
        )
        * 100
    ).astype(int)
