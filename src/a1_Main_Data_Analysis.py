...
def normal_EDA(
    ...
    data_title,
    file_type="csv",
    return_information="complex",
    header=0,
    names=[],
):
    """
    Takes in a data file and does typical exploratory data analysis
    including overview statistics, null value reports,
    visualizations, and other useful info.

    Args:
        file_path (str): Location of the file to be explored.
        data_title (str): What you want to name your data.
        file_type (str, optional):
            Input file type. Defaults to 'csv'
        return_information (str, optional):
            'basic' or 'complex' report. Defaults to 'complex'.
        header (int or None, optional):
            Pandas read file header values location parameter. Defaults to 0.
        names (list, optional):
            Pandas read file column name parameter. Defaults to [].
    ...
    if file_type != "csv":
        print(
            "normal_EDA only works with csv files currently. Support for other file types will be added in the future"
        )
        return None
    ...
    print(
        f"This dataset has {shape[0]} rows and {shape[1]} columns, creating {data_points} total data points."
    )
    null_data_points = (df.isnull().sum()).sum()
    percent_null = round((null_data_points / data_points), 3) * 100
    print(
        f"Of the {data_points} total data points, {percent_null}% or {null_data_points} are null."
    )
    ...
    percent_nulls_by_column = (
        df.isnull().sum() / df.isnull().count()
    ).sort_values(ascending=False)
    null_report = pd.concat(
        [nulls_by_column, percent_nulls_by_column],
        axis=1,
        keys=["Count", "Percent Null"],
    )
    ...
    print(" ")
    print(
        "-------------------Building and saving a cool HTML report.-------------------------------------------------------------"
    )
    print(" ")
...
if __name__ == "__main__":
    ...
    beer_advocate_df.isnull().sum()
    BA_state_df = beer_advocate_df.loc[
        :, ["brewery_id", "brewery_name", "review_overall"]
    ]
    BA_state_df.info()
    BA_state_df_agg = BA_state_df.groupby("brewery_name").agg(
        {
            "brewery_id": [
                ("brewery_id_most_common", lambda x: pd.Series.mode(x)[0]),
                "count",
    ...
    BA_state_df_2 = (
        BA_state_df.groupby("brewery_id")
        .agg(
            {
                "review_overall": lambda x: round(x.mean(), 2),
                "brewery_name": lambda column: column.iloc[0],
                "brewery_id": [("review_count", "count")],
            }
        )
        .reset_index()
    )
